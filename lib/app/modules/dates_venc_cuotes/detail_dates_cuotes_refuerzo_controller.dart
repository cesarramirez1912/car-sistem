...
import '../../data/providers/remote/sells_api.dart';
import '../../global_widgets/snack_bars/snack_bar_error.dart';
import '../../global_widgets/snack_bars/snack_bar_success.dart';

class DetailDatesCuotesRefuerzosController extends GetxController {
  ...
  void onInit() async {
    super.onInit();
    Map<String, String?> _maps = Get.parameters;
    idVenta = int.tryParse(_maps['idVenta'].toString());
    isLoading.value = true;
    await cuotesAndRefuerzosRequest();
  }

  Future<void> cuotesAndRefuerzosRequest() async {
    try {
      Future _reqCuotes = requestCuotes();
      Future _reqRefuerzos = requestRefuerzos();
      await Future.wait([_reqCuotes, _reqRefuerzos]);
    } catch (e) {
    } finally {
  ...
  Future<void> requestCuotes() async {
    List<CuoteDetailModel> _list =
        await _sellVehicleRepository.requestCuotes(idVenta);
    listaCuotes.clear();
    listaCuotes.addAll(_list);
  }

  Future<void> requestRefuerzos() async {
    List<RefuerzoDetailModel> _list =
        await _sellVehicleRepository.requestRefuerzos(idVenta);
    listaRefuerzos.clear();
    listaRefuerzos.addAll(_list);
  }
}
